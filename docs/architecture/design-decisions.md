...
| **Conservative GC** | GC may delete only confirmed `orphaned` items. `attached`, `detached`, and `unreachable` items are protected. Repository store directories are not deleted merely because a local clone is missing. |
...
| **Store-level advisory file lock** | A lock on `<store>/.lock` prevents concurrent writes from interleaving index and manifest updates. |
