...
/// Returns the set of repo-relative paths of files tracked by git within
/// `abs_dir`.
///
/// Runs `git ls-files -- <abs_dir>` once, which is more efficient than
/// calling [`is_tracked`] per file.  Paths in the returned set use the same
/// forward-slash, repo-root-relative format as [`Item::path`].
...
///
/// This intentionally accepts only common HTTPS/SSH/scp-like Git remotes. The
/// returned value is a ranking hint, never proof of identity.
