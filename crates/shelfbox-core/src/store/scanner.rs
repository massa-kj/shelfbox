...
/// Result of a non-mutating scan over `<store>/repos/*/manifest.json`.
...
/// A successfully parsed repository manifest and its store directory name.
#[derive(Debug)]
...
/// Non-fatal scanner errors. Callers decide whether each error should warn,
/// skip, or abort the higher-level operation.
#[derive(Debug)]
pub enum ScanError {
    ReadFailed {
        dir: String,
...
/// Scans repository manifests under `<store_root>/repos/` without modifying
/// any store files.
...
mod tests {
    use super::*;
    use crate::store::manifest::{Item, OwnershipState};
